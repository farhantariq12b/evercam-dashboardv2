      ...
      >
        <v-row style="max-widht: 100%">
          ...
          <v-col cols="12" class="pt-0">
            <v-row class="pl-2 pr-1">
              ...
              >
                <img
                  v-if="slides[imageIndex]"
                  :src="slides[imageIndex]"
                  :class="{ highlight: imageIndex == index }"
                  style="max-width: 100%"
                >
                <div
                  v-else
        ...
        <v-row>
          ...
          <v-col cols="4" class="pt-0 float-right">
            <v-dialog v-model="dialog" width="500">
              <template v-slot:activator="{ on }">
                <v-btn color="primary" text class="float-right" v-on="on">
              ...
              <v-card>
                <v-card-text>
                  <v-tabs
                    ...
                    :right="true"
                  >
                    <v-tab v-for="item in ['Schedule', 'Advanced']" :key="item">
                      {{ item }}
                    </v-tab>
                  </v-tabs>
                        ...
                        <v-card-text>
                          <v-row justify="space-around" no-gutters>
                            ...
                            <v-col class="d-flex" cols="12" sm="12">
                              ...
                              >
                                &nbsp;&nbsp;seconds,
                                {{ custom_duration * 24 }} Frames playing at 24
                                FPS.
                              </span>
                              <span v-else class="strong font-weight-normal">&nbsp;&nbsp;{{ duration * 24 }} Frames playing
                                at 24 FPS.</span>
                              <span
                                v-if="headers"
                                class="strong font-weight-normal"
                              >&nbsp;&nbsp;(+ 4 seconds intro + 6 seconds
                                outro)</span>
                            </v-col>
          ...
          <v-col cols="3" class="pt-0 float-right">
            <v-btn width="100%" color="primary" dark @click="checkForm">
              Create Time-lapse
            </v-btn>
          </v-col>
        </v-row>
...
export default {
  ...
  methods: {
    ...
    checkForm() {
      if (
        moment(this.to_datetime2).isBefore(this.from_datetime2) ||
        moment(this.to_datetime2).isSame(this.from_datetime2)
      ) {
        ...
        this.$axios
          ...
          .then(() => {
            ...
            this.snackbarText =
              "Your request has been processed. We are preparing your time-lapse"
            this.snackbar = true
          })
          .catch(() => {
            this.snackbarColor = "error"
