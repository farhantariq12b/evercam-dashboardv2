<template>
  <v-layout class="pr-2 pl-2">
    ...
    <v-container>
      <v-row cols="12">
        <v-col md="4">
          <v-btn color="primary" text @click="backTab">
            <v-icon class="pr-2">
              fas fa-arrow-left
            </v-icon>Back
          </v-btn>
        ...
        <v-col md="1" class="ml-auto">
          <v-btn color="primary" class="float-right" text @click="backTab">
            <v-icon>fas fa-trash-alt</v-icon>
          </v-btn>
        </v-col>
      </v-row>
      <v-row ref="rowWith" cols="12">
        <v-layout align-center justify-center>
          <v-btn color="secundary" text :disabled="undoDisabled" @click="undo">
            <v-icon>fas fa-undo-alt</v-icon>
          </v-btn>
          <v-btn color="secundary" text :disabled="redoDisabled" @click="redo">
            <v-icon>fas fa-redo-alt</v-icon>
          </v-btn>
          <v-btn color="secundary" text @click="stopDrawing">
            <v-icon>fas fa-hand-paper</v-icon>
          </v-btn>
          <v-btn
            color="secundary"
            text
          ...
          <v-btn color="secundary" text @click="addArrow">
      ...
      <v-row cols="12">
        <v-col md="4" class="pb-0">
          ...
          <div v-if="editTitle">
            ...
            <v-icon color="primary" size="18" @click="editTitle = !editTitle">
              fas fa-times
            </v-icon>
          </div>
          <v-icon
      ...
      <v-row cols="12">
        <v-col md="12">
          <p class="subtitle font-weight-bold float-left pr-2">
            Requested by:
          </p><p class="subtitle">
            {{ firstname }} {{ lastname }}
          </p>
...
export default {
  ...
  mounted: function() {
    this.$refs.tuiImageEditor
      .invoke("loadImageFromURL", this.selectedImage.data, "My sample image")
      .then(data => {
        var height =
          (data.newHeight * this.$refs.rowWith.clientWidth) / data.newWidth
        this.height = height
        this.$refs.tuiImageEditor
          .invoke("resizeCanvasDimension", {
            width: this.$refs.rowWith.clientWidth,
            height: height
          })
          .then(data => {
